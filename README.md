...
This version of GinJinn is discontinued. See [GinJinn2](https://github.com/AGOberprieler/GinJinn2) for the recent version.
...
GinJinn simplifies the management of object detection projects by automatically converting annotated images to TensorFlow record format and setting up model pipeline configurations, and provides an easy interface to training and export functionality. Additionally, GinJinn provides a command for instantly using trained and exported models for the detection of objects in new data.
...
We highly recommend using GinJinn GPU for this analysis; the CPU version will most likely be prohibitively slow. If you do not have a sufficient GPU and still want to reproduce this analysis you can use a computationally less demanding model (will be noted in the following steps), which will only require around 24 hours to train, but will also lead to worse results.
